...
extern "C" void __declspec(dllexport) Set(HWND hwndParent, int string_size, TCHAR *variables, stack_t **stacktop)
{
...

  {
	...
	hr=SHGetPropertyStoreFromParsingName(wszPath, NULL, GPS_READWRITE, IID_PPV_ARGS(&m_pps));
    if (SUCCEEDED(hr))
    {
      PROPVARIANT propvar;
	  hr=InitPropVariantFromString(wszAppID, &propvar);
      if (SUCCEEDED(hr)) {
		hr=m_pps->SetValue(PKEY_AppUserModel_ID, propvar);
        if (SUCCEEDED(hr)) {
          if (dualMode) {
          ...
          if (SUCCEEDED(hr)) {
            success = true;
    pushstring(szAppID, MAX_PATH);
          }
		else
		{
		hrStr <<"04_"<<hr << "_hr";
		pushstring(hrStr.str().c_str(), MAX_PATH);
		}
        }
	else
	{
